...
cnn.init(in_dim=i_ar([channels]), in_nb_ch=3, out_dim=nb_mol,
         bias=0.1, b_size=32, comp_meth="C_BLAS", dynamic_load=1,
         mixed_precision="FP32C_FP32A")  # initialization of the CNN backbone

cnn.create_dataset("TEST", size=i_ar(nb_test), input=f_ar(data),
                   target=f_ar(target * 0))  # loading of the test dataset in CIANNA

load_iteration = 99  # model from the 99th iteration
cnn.load(model_path + "net0_s%04d.dat"%load_iteration, load_iteration, bin=1)  # CIANNA loads the CNN-model with all the weights
